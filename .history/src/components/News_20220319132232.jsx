...
//   // }
  ...
  const fetchMoreData = async() => {
    ...
    let data = await fetch(url);
    let pareseData = await data.json()
    setArticales(articles.concat(pareseData.articles)
    setTotalReasult(pareseData.totalReasult)
  
  //   this.setState({  class base.......
  ...
  // render() {
    return (
      <>
        <h2 className="text-center"style={{margin:'35px 0px'}}>Latest News - Top Headlines from {capitalizeFirstLetter(props.category)}</h2>
      {loading && <Spiner />}
      <InfiniteScroll
          dataLength={articles.length}
          next={fetchMoreData}
          hasMore={articales.length !==totalReasult}
          loader={<Spiner/>}>
