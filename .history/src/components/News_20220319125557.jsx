...
//   // }
  ...
  const url =`https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=74d2dd5553924d7c9adadba75aa5208d&page=${state.page}&pageSize=${props.pageSize}`;
  this.setState({loading:true});
    let data = await fetch(url);
    props.setProgress(30);
    let pareseData = await data.json()
    props.setProgress(70);
    setArticales(pareseData.articles)
  ...
  fetchMoreData = async() => {
    this.setState({page: this.state.page  + 1})
    const url =`https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=74d2dd5553924d7c9adadba75aa5208d&page=1&pageSize=${props.pageSize}`;
    let data = await fetch(url);
    let pareseData = await data.json()
    this.setState({
  ...
  // render() {
    return (
      <>
        <h2 className="text-center"style={{margin:'35px 0px'}}>Latest News - Top Headlines from {this.capitalizeFirstLetter(props.category)}</h2>
